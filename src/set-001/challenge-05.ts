...
import { bytesToHex, repeatingKeyXor, strToBytes } from "../crypto"
...
function start() {
  ...
  const encrypted: Uint8Array = repeatingKeyXor(strToBytes(input), strToBytes(key))

  console.log(bytesToHex(encrypted))
  console.assert( expectedOut === bytesToHex(encrypted), 'your repeating key xor didn\'t work' )
}
